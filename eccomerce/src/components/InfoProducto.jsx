        ...
        <div>
            <div className='max-w-screen-lg mx-auto my-10 flex max-[740px]:flex-col gap-10 justify-center items-center'>
                <p className='font-bodyFont text-4xl font-bold min-[740px]:hidden'>
                    {location.state.item.Detalle_Producto}
                </p>
                <div className='w-3/5 md:w-2/5 relative'>
                    <img className='w-full h-[400px] object-cover' src={`data:image/jpeg;base64,${location.state.item.image}`} alt={location.state.item.Detalle_Producto} />
                    ...
                    <div className='top-5 right-0 absolute'>
                        <p className='bg-black text-white font-semibold font-bodyFont py-1 px-8'>
                            {location.state.item.Marca_Producto.Nom_Marca}
                        </p>
                ...
                <div className='w-3/5 flex flex-col justify-center gap-12'>
                    <div>
                        ...
                        <div className='flex items-center gap-5 mt-3'>
                            <p className='text-2xl font-mono'>Precio: ${location.state.item.Precio_Producto}</p>
                    ...
                    <div className='flex max-[740px]:flex-col gap-5 items-center'>
                        ...
                        <p className='text-base text-gray-500'>
                            ...
                            <span className='font-medium capitalize pl-1'>
                                {location.state.item.Categoria_Producto.Nom_Categoria}
                            </span>
                        </p>
                        <p className='text-base text-gray-500'>
                            Marca:
                        ...
                        <p className='text-base text-gray-500'>
                            Color:
                            <span className='font-medium capitalize pl-1'>
                                {location.state.item.Color_Producto.Nom_Color}
                            </span>
                        </p>
                        <p className='text-base text-gray-500'>
                            Talle:
                            <span className='font-medium capitalize pl-1'>
                                {location.state.item.Talle_Producto.Nom_Talle}
                            </span>
                        </p>
                    </div>
                    {/* location.state.item del producto */}
                        ...
                        <div className='w-52 flex items-center justify-between gap-4 text-gray-500 border p-3 shadow-md'>
                            ...
                            <div className='flex item gap-4 text-sm font-semibold '>
                                ...
                                duration-300 active:bg-black'
                                    ...
                                    onClick={() => setBaseCant(baseCant === location.state.item.Existencia_Producto ? baseCant : baseCant + 1)}
                                >+</button>
                            </div>
                        </div>

                        <button className='bg-gray-700 text-white gap-1 flex py-3 px-6  active:bg-orange-700' onClick={() => dispatch(addToCart({
                            id: location.state.item.id_Producto,
                            title: location.state.item.Detalle_Producto,
                            image: location.state.item.Img_Producto,
                            marca: location.state.item.Marca_Producto.Nom_Marca,
                            price: location.state.item.Precio_Producto,
                            cantidad: baseCant
                        })) & toast.success(`se agrego ${baseCant} ${location.state.item.Detalle_Producto} al carro`)}>add</button>
                        <div className='absolute  '>

                            {
                                baseCant === location.state.item.Existencia_Producto && <span className='flex text-center text-xs text-red-600'>(maximo disponible)</span>
                            }
                        </div>
